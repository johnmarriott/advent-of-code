...
robot would prefer to do the same motion consecutively, e.g., 
up left up is more expensive in key presses than up up left
"""
...
def move_numeric_keypad(current_digit: str, next_digit: str) -> int:
    ...
    for path in paths:
        ...
        for next_first_arrow_keypad in first_arrow_keypad_code:
            path_cost += move_arrow_keypad(current_first_arrow_keypad, next_first_arrow_keypad, 24)
...
@cache
def move_arrow_keypad(current_digit: str, next_digit: str, layer: int) -> int:
    """
    returns the number of human key presses needed to move the 
    arrow keypad at this layer from current_digit to next_digit 
    """

    current_next_arrow_keypad = "A"
    min_cost = float("inf")
    ...
    for path in paths:
        ...
        next_arrow_keypad_code = [
            arrow_keypad[this_arrow_keypad_step[0]][this_arrow_keypad_step[1]]["label"]
            for this_arrow_keypad_step
            in zip(path, path[1:])
        ] + ["A"]

        if layer == 0:
            min_cost = min(min_cost, len(next_arrow_keypad_code))
        else:
            for next_second_arrow_keypad in next_arrow_keypad_code:
                path_cost += move_arrow_keypad(current_next_arrow_keypad, next_second_arrow_keypad, layer - 1)
                current_next_arrow_keypad = next_second_arrow_keypad
